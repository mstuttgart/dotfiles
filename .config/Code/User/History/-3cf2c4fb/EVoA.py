...
class TestNFeDSF(TestBaseBr):
    ...
    def test_nfse_homologacao_sucesso(self, recepcionar_lote_rps, consultar_situacao_lote, consultar_lote_rps, cancelar_nfse):
        ...
        for invoice in self.invoices:
            ...
            with open(os.path.join(self.caminho, 'xml', 'enviar_lote_rps_resposta.xml')) as xml:
            ...
            with open(os.path.join(self.caminho, 'xml', 'consultar_situacao_rps_resposta_2.xml')) as xml:
                xml_recebido = xml.read().replace('\n', '')
            ...
            self.assertEqual(elec_inv.mensagem_retorno, 'Lote aguardando processamento')  # noqa
            ...
            # Consulta do resultado do lote
            with open(os.path.join(self.caminho, 'xml', 'consultar_situacao_rps_resposta_4.xml')) as xml:
                xml_recebido = xml.read().replace('\n', '')
            ...
            # Consulta do resultado do lote
            with open(os.path.join(self.caminho, 'xml', 'consultar_lote_rps_resposta.xml')) as xml:
                xml_recebido = xml.read().replace('\n', '')
            ...
            self.assertEqual(elec_inv.mensagem_retorno, 'NFSe emitida com sucesso')  # noqa
            ...
            # Consulta do resultado do lote
            with open(os.path.join(self.caminho, 'xml', 'cancelar_nfse_resposta.xml')) as xml:
                xml_recebido = xml.read().replace('\n', '')
    ...
    def test_nfse_consultar_situacao_lote_erro(self, recepcionar_lote_rps, consultar_situacao_lote):
        ...
        for invoice in self.invoices:
            ...
            self.assertEqual(elec_inv.mensagem_retorno, 'Aguardando envio')  # noqa
    ...
    def test_nfse_consulta_lote_rps_erro(self, recepcionar_lote_rps, consultar_situacao_lote, consultar_lote_rps):
        ...
        for invoice in self.invoices:
            ...
            # Consulta do resultado do lote
            with open(os.path.join(self.caminho, 'xml', 'consultar_situacao_rps_resposta_3.xml')) as xml:
                xml_recebido = xml.read().replace('\n', '')
            ...
            # Consulta do resultado do lote
            with open(os.path.join(self.caminho, 'xml', 'consultar_lote_rps_resposta_erro.xml')) as xml:
                xml_recebido = xml.read().replace('\n', '')
            ...
            self.assertEqual(elec_inv.mensagem_retorno, 'Valor líquido de NFSe informada incorretamente')  # noqa

            events = elec_inv.electronic_event_ids

            self.assertEqual(len(events), 1)
            self.assertEqual(events[0].code, 'E181')
            self.assertEqual(events[0].name, 'Valor líquido de NFSe informada incorretamente')  # noqa
            self.assertEqual(events[0].category, 'info')
    ...
    def test_nfse_consulta_lote_rps_erro_E10(self, recepcionar_lote_rps, consultar_situacao_lote, consultar_lote_rps, consultar_nfse_por_rps):
        ...
        for invoice in self.invoices:
            ...
            with open(os.path.join(self.caminho, 'xml', 'consultar_lote_rps_resposta_erro_E10.xml')) as xml:
                xml_recebido = xml.read().replace('\n', '')
            ...
            # Realizamos o mock do metodo 'consultar_lote_rps'
            with open(os.path.join(self.caminho, 'xml', 'consultar_nfse_por_rps_resposta_erro_E10.xml')) as xml:
                xml_recebido = xml.read().replace('\n', '')
            ...
            self.assertEqual(elec_inv.state, 'done')
            self.assertEqual(elec_inv.codigo_retorno, '100')
            self.assertEqual(elec_inv.mensagem_retorno, 'Nota Fiscal emitida com sucesso')  # noqa
