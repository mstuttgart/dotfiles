    ...
    def post_pdv_venda(self, context=None, **kw):
        ...
        try:
            ...
            for item in json.loads(values['items']):
                ...
                lines.append((0, 0, {
                    'codigo_produto': item.get(item['codigo_produto']),
                    'nome_produto': item.get(item['nome_produto']),
                    'quantidade': item.get(item['quantidade']),
                    'preco': item.get(item['preco']),
                    'desconto': item.get(item['desconto']),
                    'total_liquido': item.get(item['total_liquido')]
                }))
