...
class Channel(http.Controller):
    ...
    #         pos_order (pos.order.entry): pedido onde o json sera anexado
    ...
    #         'config_guid': data.get('config_guid', ''),
    ...
    def post_pdv_venda(self, context=None, **kw):
        ...
        try:
            ...
            entries = request.env['pos.order.entry'].with_context(ctx).search([
            ...
            if entries:
                ...
                result = {
                    'message': f"Venda {values.get('numero_venda', '')} já registrada no sistema.",
                }
