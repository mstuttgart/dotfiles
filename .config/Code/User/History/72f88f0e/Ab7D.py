...
class Channel(http.Controller):
    ...
    def post_pdv_venda(self, context=None, **kw):
        ...
        if request.env['pos.order'].with_context(ctx).search([('name', '=', values['numero_venda'])]):
        ...
        try:
            ...
            for item in json.loads(values['items']):
                ...
                lines.append((0, 0, {
                    'qty': item['quantidade'],
                    'price_unit': item['preco'],
                    'price_subtotal': item['preco'],
                    'price_subtotal_incl': item['total_liquido'],
            ...
            vals = {
                ...
                'total_bruto': values['valor_venda'],
                'total_desconto': values['valor_desconto'],
                'amount_total': values['valor_liquido'],
                'amount_paid': 0,
                'amount_tax': 0,
                'amount_return': 0,
                'lines': lines,
            ...
            domain = ['|', ('cnpj_cpf', '=', val), ('cnpj_cpf', '=', cnpj_cpf)]  # noqa
