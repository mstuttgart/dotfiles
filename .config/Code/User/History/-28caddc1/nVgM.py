...
class ResCompany(models.Model):
    ...
    def action_post_tecnospeed_logotipo(self):
        """Realiza o POST e PUT para obter/atualizar
        o ID do logotipo no PlugNotas (tecnospeed). O ambiente
        ...
        Raises:
            ...
            UserError: Erro retornado pelo PlugNotas, como ID de logotipo inexistente, por exemplo.
        """

        # ambiente de desenvolvimento sandbox
        if self.tecnospeed_sandbox_active:

            # atualiza ID logotipo
            if self.tecnospeed_logotipo_id_sandbox:
                action = 'PUT'
                request_vals = self.get_tecnospeed_request_vals(subdomain=f'logotipo/{self.tecnospeed_logotipo_id_sandbox}', content_type=None)

            # cadastra logotipo e obtem seu ID
            else:
                action = 'POST'
                request_vals = self.get_tecnospeed_request_vals(
                    subdomain='logotipo', content_type=None)

        else:

            # atualiza ID logotipo
            if self.tecnospeed_logotipo_id_producao:
                action = 'PUT'
                request_vals = self.get_tecnospeed_request_vals(subdomain=f'logotipo/{self.tecnospeed_logotipo_id_producao}', content_type=None)
    ...
    def action_patch_tecnospeed_company(self):
        """Realiza o PATCH para obter/atualizar
        o cadastro da empresa no PlugNotas (tecnospeed). O ambiente
        selecionado depende se o ambiente de 'sandbox' esta ou não ativo.
    ...
    def _prepare_company_values(self):
        ...
        values = {
            ...
            "endereco": {
                ...
                "tipoLogradouro": "Rua",
                "codigoPais": self.country_id.ibge_code,
