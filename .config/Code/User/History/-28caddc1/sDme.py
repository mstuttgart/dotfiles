...
class ResCompany(models.Model):
    ...
    def action_post_tecnospeed_logotipo(self):
        """Realiza o POST do logotipo. O ambiente selecionado depende se o
        ambiente de 'sandbox' esta ou não ativo.

        Raises:
            UserError: Erro de conexão com o ambiente do PlugNotas
            UserError: Erro no upload do logotipo
        """

        action = 'POST'
        request_vals = self.get_tecnospeed_request_vals(subdomain=f"empresa/{re.sub('[^0-9]', '', self.cnpj_cpf or '')}/logotipo", content_type=None)

        files = [
            ('arquivo', ('logotipo.jpg', base64.b64decode(self.logo), 'application/octet-stream'))
        ]
    ...
    def action_post_tecnospeed_company(self):
        ...
        self.action_post_tecnospeed_logotipo()
