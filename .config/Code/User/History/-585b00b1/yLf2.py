...
class InvoiceElectronic(models.Model):
    ...
    def _prepare_electronic_invoice_values(self):
        ...
        # CNAE e Inscr. Municipal apenas para NFe Conjugada``
        if self.company_id.cnae_main_id and self.company_id.inscr_mun and self.fiscal_position_id.position_type == 'conjugate':
        ...
        if self.transportadora_id:
            xender = f"{self.transportadora_id.street or ''} - {self.transportadora_id.number or ''}, {self.transportadora_id.district or ''}"[:60]
        ...
        transp = {
            ...
            'transporta': {
                'xNome': (self.transportadora_id.legal_name or self.transportadora_id.name or ''),
                'IE': re.sub('[^0-9]', '', self.transportadora_id.inscr_est or ''),
                'xEnder': xender,
    ...
    def _get_protocolo_nfe(self):
        ...
        for rec in self:
            ...
            values = {
                ...
                'data_autorizacao': response_obj.protNFe.infProt.dhRecbto,
            }
    ...
    def action_cancel_document(self, context=None, justificativa=None):
        ...
        if self.model in ('55', '65'):
            ...
            # Ajusta data para a timezone do usuario
            dh_evento = pytz.utc.localize(fields.Datetime.now(self)).astimezone(tz)
            ...
            cancelamento = {
                ...
                'eventos': [{
                    ...
                    'dhEvento': dh_evento.strftime('%Y-%m-%dT%H:%M:%S-03:00'),
