...
# 
        tasks = odoo.execute('project.task', 'browse', task_ids)

        # tasks = odoo.env['project.task'].browse([task_ids])

        # import ipdb; ipdb.set_trace()  # noqa

        for task_id in task_ids:
            # browse do RPC nao aceita recordset multip
            task_obj = odoo.env['project.task'].browse(task_id)
            task_obj._onchange_stage_id()
        

        # Encerramos o ticket da tarefa, caso exista
        # for task in odoo.env['project.task'].browse(task_ids):
