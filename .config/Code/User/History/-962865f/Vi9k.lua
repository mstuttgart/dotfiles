...
return {
  ...
  -- buffer + tab line
  {
    ...
    keys = {
      ...
      { 'b]',         '<Cmd>BufferLineCyclePrev<CR>',            desc = 'Previous Buffers' },
    },
    config = function()
  ...
  -- comment plugin
  {
    ...
    init = function()
      vim.keymap.set('n', '<leader>/', require("Comment.api").toggle.linewise.current(), { desc = 'Toggle Comment Line' })
      vim.keymap.set('n', '<leader>/', <ESC><cmd>lua require('Comment.api').toggle.linewise(vim.fn.visualmode())<CR>, { desc = 'Toggle Comment Line' })
