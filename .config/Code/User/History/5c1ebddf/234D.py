    ...
    def action_get_electronic_invoice_status(self):
        ...
        for rec in self:
            ...
            if rec.model == '001' and rec.recibo_nfe and rec.webservice_nfse == 'nfse_ginfes' and rec.state in ['open', 'error']:
                ...
                if "Situacao" in dir(ret_rec):
                    ...
                    if ret_rec.Situacao in (3, 4):
                        ...
                        # RPS ja emitido
                            ...
                            vals.update({
                                'state': 'error',
                                'codigo_retorno': ret_lote.ListaMensagemRetorno.MensagemRetorno.Codigo,
                                'mensagem_retorno': ret_lote.ListaMensagemRetorno.MensagemRetorno.Mensagem,
                            })
                ...
                # Empresa ja com uma consulta em andamento.
                # Favor aguardar o termino de uma consulta para efetuar outra.
                elif ret_rec.ListaMensagemRetorno.MensagemRetorno.Codigo == 'A02':

                    # Deixamos status em 'open' para que a consulta possa ser refeita
                    # posteriormente
                    vals.update({
                        'state': 'open',
