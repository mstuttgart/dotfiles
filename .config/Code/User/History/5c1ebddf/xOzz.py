...
class InvoiceElectronic(models.Model):
    ...
    def action_send_electronic_invoice(self):
        ...
        for rec in self:
            ...
            if rec.model == '001' and rec.webservice_nfse == 'nfse_ginfes' and rec.state == 'draft':
                ...
                # fortaleza
                if rec.company_id.city_id == self.env.ref('br_base.city_2304400'):
                    emissor = 'fortaleza'
                else:
                    emissor = 'ginfes'

                xml_enviar = pytrustnfe.nfse.ginfes.xml_recepcionar_lote_rps(certificado, nfse=nfse_values, emissor=emissor)  # noqa
    ...
    def action_get_electronic_invoice_status(self):
        ...
        for rec in self:
            ...
            if rec.model == '001' and rec.recibo_nfe and rec.webservice_nfse == 'nfse_ginfes' and rec.state in ['open', 'error']:
                ...
                obj = {
                    'cnpj_prestador': re.sub('[^0-9]', '', rec.company_id.cnpj_cpf),  # noqa
                    'inscricao_municipal': re.sub('[^0-9]', '', rec.company_id.inscr_mun),  # noqa
                ...
                certificado = pytrustnfe.certificado.Certificado(cert_pfx, rec.company_id.nfe_a1_password)  # noqa
                ...
                try:
                    ...
                    consulta_situacao = pytrustnfe.nfse.ginfes.consultar_situacao_lote(
                        certificado, consulta=obj, ambiente=rec.ambiente, emissor=emissor)
                ...
                if "Situacao" in dir(ret_rec):
                    ...
                    vals.update({
                        'state': 'error',
                        'codigo_retorno': ret_rec.ListaMensagemRetorno.MensagemRetorno.Codigo,  # noqa
                        'mensagem_retorno': ret_rec.ListaMensagemRetorno.MensagemRetorno.Mensagem,  # noqa
                    })
                ...
                if consulta_lote:
                    self._create_attachment('rec', rec, consulta_lote['sent_xml'])  # noqa
                    self._create_attachment('rec-ret', rec, consulta_lote['received_xml'])  # noqa
    ...
    def action_cancel_document(self, context=None, justificativa=None):
        ...
        if self.model == '001' and self.webservice_nfse == 'nfse_ginfes' and self.state == 'done':
            ...
            try:
                if self.company_id.city_id == self.env.ref('br_base.city_2304400'):
                    emissor = 'fortaleza'
                else:
                    emissor = 'ginfes'
                ...
                cancel = pytrustnfe.nfse.ginfes.cancelar_nfse(
                    certificado, cancelamento=canc, ambiente=self.ambiente, emissor=emissor)
    ...
    def _consultar_nfse_rps_ginfes(self):
        ...
        try:
            ...
            consulta_nfse = pytrustnfe.nfse.ginfes.consultar_nfse_por_rps(
                certificado, consulta=consulta, ambiente=self.ambiente, emissor=emissor)
