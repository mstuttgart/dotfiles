...
class PosOrderEntry(models.Model):
    ...
    def unlink(self):
        ...
        for rec in self:
            if rec.state in ('done', 'open'):
                raise UserError(_("Apenas registros com status diferente de 'Finalizado' podem ser excluídos."))
    ...
    def action_back_to_draft(self):
        ...
        for rec in self:
            ...
            if rec.state in ('open', 'error'):
                ...
                raise UserError(_('Somente requisições com status diferente de "Finalizado" podem retornar para Provisório'))
    ...
    def action_cancel_entry(self):
        ...
        for rec in self:
            ...
            if rec.state in ('draft', 'error'):
                ...
                raise UserError(_('Somente entradas com status "Provisório" ou "Erro" podem ser canceladas'))
    ...
    def action_confirm_entry(self):
        ...
        for rec in self:
            ...
            if rec.state == 'draft':
                ...
                raise UserError(_('Somente entradas com status "Provisório" podem ser confirmadas'))
    ...
    def action_convert_to_pos_order(self):
        ...
        for rec in self:
            ...
            pos_config = self.env['pos.config'].sudo().search([('guid', '=', rec.config_guid)], limit=1)
            ...
            if not pos_config.pricelist_id or not pos_config.default_fiscal_position_id or not pos_config.stock_location_id:

                POS_ORDER_ENTRY_EVENT.create({
                    'category': 'error',
                    'name': 'Os campos "Posição Fiscal", "Lista de Preços e "Localização do Estoque" da Config. do POS são obrigatórios.',
                    'pos_order_entry_id': rec.id,
                })

                continue

            pos_session = self.env['pos.session'].sudo().search([('guid', '=', rec.session_guid)], limit=1)
            ...
            order = self.env['pos.order'].sudo().create(vals)
    ...
    def cron_convert_pos_order_entry(self):
        ...
        for entry in entries:
            entry.with_delay().action_convert_to_pos_order()
