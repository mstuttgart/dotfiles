...
with open('municipios.csv', 'r', encoding='utf-8') as t1, open('res.state.city.csv', 'r', encoding='utf-8') as t2:
    ...
    with open('update.csv', 'w', encoding='utf-8') as out_file:
        writers = csv.writer(out_file, lineterminator='\n')

        all = []
        header = next(ibge_csv)
        header.append('mds_code')
        all.append(header)
        ...
        for linha in ibge_csv:
            ...
            for linha_mds in mds_csv:
                ...
                # print(f"'{linha_mds[1].strip()}'")
                    print(f"'{linha[5]} e {linha_mds[1].strip()}'")
                    linha.append(linha_mds[0])
                    continue
