...
class BrZipAbstract(models.AbstractModel):
    ...
    def get_address_from_zip(self, zip_code):
        ...
        try:
            ...
            return {
                'search_cep_exception': f'CEP buscado é inválido: {zip_code}',
            }

        except brazilcep.exceptions.CEPNotFound:
            return {
                'search_cep_exception': f'CEP {zip_code} não encontrado',
            }

        except requests.exceptions.ConnectionError as errc:
            return {
                'search_cep_exception': f'Erro de conexão com o servidor de CEPs. Por favor, tente mais tarde. {errc}',
            }

        except requests.exceptions.Timeout as errt:
            return {
                'search_cep_exception': f'Tempo de consulta expirado: {errt}. Contate o administrado do sistema',
            }

        except requests.exceptions.HTTPError as errh:
            return {
                'search_cep_exception': f'Erro na requisição HTTP: {errh}. Contate o administrado do sistema',
            }

        except Exception as exc:
            return {
                'search_cep_exception': f'Ocorreu um erro desconhecido. Contate o administrado do sistema. Erro: {exc}',
            }
    ...
    def _get_address(self, zipcode):
        ...
        if not br_zip:
            ...
            if 'search_cep_exception' in address:
                values = {
                    'street': '',
                    'street2': '',
                    'number': '',
