...
class BrZipAbstract(models.AbstractModel):
    ...
    def get_address_from_zip(self, zip_code):
        ...
        try:
            ...
            raise UserError(f'Erro de conexão com o servidor de CEPs. Por favor, tente mais tarde. {errc}')

        except requests.exceptions.Timeout as errt:
            raise UserError(f'Tempo de consulta expirado: {errt}. Contate o administrado do sistema')

        except requests.exceptions.HTTPError as errh:
            raise UserError(f'Erro na requisição HTTP: {errh}. Contate o administrado do sistema')

        except Exception as exc:
            raise UserError(f'Ocorreu um erro desconhecido. Contate o administrado do sistema. Erro: {exc}')
