...
VERSION = subprocess.run('git describe --abbrev=0', shell=True,
                         stdout=subprocess.PIPE, check=False).stdout.decode('utf8')[:-1]
...
# Monstamos os paths principais do projeto
SERVER_PATH = f'{ADDONS_PATH}/core/odoo-bin'
...
def foreach(command):
    ...
    """
    cmd = f'git submodule foreach "{command}"'
    click.secho(click.style(f'[INFO] Running {cmd}\n', fg='green'))
...
def checkout(ctx, branch):
    ...
    """
    ctx.invoke(foreach, command=f'git checkout {branch} | true && echo')
...
def clean(ctx, branch):
    ...
    """
    command = f"git branch | grep \'{branch}-T[0-9]\' | xargs git branch -D || true && echo"
    ctx.invoke(foreach, command=command)
...
def create_release(ctx, tag):
    ...
    - rebase 'main 'branch with content of 'develop' branch for all submodules;\n
    ...
    for p in addons_path:
        ...
        subprocess.call(f'git add {}' % p, shell=True)
    ...
    if click.confirm('\nDo you want to continue and create release?', abort=True):
        ...
        subprocess.call('git tag -a %s' % tag, shell=True)
...
def create_hotfix(ctx, tag):
   ...
   Arguments:
    ...
    # Adiciona alteracoes em libs
    subprocess.call(f'git add {LIBS_PATH}', shell=True)
    ...
    if click.confirm('\nDo you want to continue and create hotfix?', abort=True):
        ...
        subprocess.call(f'git tag -a {tag}', shell=True)
        subprocess.call('git push --tags', shell=True)
...
def odooconf():
    ...
    # Renderiza template
    with open(odoo_cfg_path, 'w', encoding="utf-8") as odoo_cfg_file:
...
@click.pass_context
def todev(database, host, user, password):
    ...
    click.secho(click.style(f'[INFO] Database {database} updated', fg='green'))  # noqa
...
def scaffold(name):
    ...
    try:
        ...
        click.secho(click.style(
            f'[INFO] Module \'{name}\' created!!', fg='green'))
