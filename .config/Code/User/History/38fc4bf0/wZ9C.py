...
class InvoiceElectronic(models.Model):
    ...
    data_cancelamento = fields.Char(string='Data de Cancelamento',
                                    size=30,
                                    readonly=True,
                                    states=STATE)
    ...
    def _create_attachment(self, prefix, event, data, extension='xml'):
        file_name = '%s-%s.%s' % (prefix, datetime.now().strftime('%Y-%m-%d-%H-%M'), extension)  # noqa

        self.env['ir.attachment'].create(
            {
                'name': file_name,
                'datas': base64.b64encode(data.encode()) if extension != 'pdf' else base64.urlsafe_b64decode(data),
