...
class InvoiceElectronic(models.Model):
    ...
    def action_send_electronic_invoice(self):
        """Realiza o envio de doc. eletronicos de NFSe. A Através do PlugNotas
        O envio é realizado de forma assincrona.
        """
                    ...
                    if rec.state == 'done':
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                rec.tecnospeed_xml = base64.b64encode(response.content)
                                rec.tecnospeed_xml_name = f'{self.numero} - NFSe - {self.partner_id.name}.xml'
                                self._create_attachment('nfse-xml', rec, response.text, extension='xml')
                            ...
                            if response.status_code == 200:
                                # salva PDF na NFSe
                                rec.tecnospeed_danfe = base64.b64encode(response.content)  # noqa
                                rec.tecnospeed_danfe_name = f'{self.numero} - NFSe - {self.partner_id.name}.pdf'
    ...
    def action_print_einvoice_report(self):
        """Realiza a impressao do PDF do DANFSe. O Edoc precisa estar
        emitido para que o PDF do DANFE esteja presente no edoc.

        Returns:
            dict: dicionario com atributos do ir.action.act_url para impressao no browser
        """

        res = super(InvoiceElectronic, self).action_print_einvoice_report()

        if self.model == '001' and self.nfe_engine == 'tecnospeed' and self.state in ['done', 'cancel'] and self.tecnospeed_danfe:

            base_url = self.env['ir.config_parameter'].get_param('web.base.url')

            file_url = f"{base_url}web/content?model=invoice.electronic&field=tecnospeed_danfe&filename_field=tecnospeed_danfe_name&id={self.id}"  # noqa
